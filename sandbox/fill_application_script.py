...
async def run_generated_script():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # --- Step 1 ---
            # Action 1
            print(f"Navigating to: https://job-boards.greenhouse.io/andurilindustries/jobs/4673813007?gh_jid=4673813007&gh_src= (Step 1, Action 1)")
            await page.goto("https://job-boards.greenhouse.io/andurilindustries/jobs/4673813007?gh_jid=4673813007&gh_src=", timeout=5000)
            await page.wait_for_load_state('load', timeout=5000)
            await page.wait_for_timeout(1000)

            # --- Step 2 ---
            # Action 2
            print(f"Scrolling down by one page height (Step 2, Action 1)")
            await page.evaluate('window.scrollBy(0, window.innerHeight)')
            await page.wait_for_timeout(500)
            # Action 3
            print(f"Scrolling down by one page height (Step 2, Action 2)")
            await page.evaluate('window.scrollBy(0, window.innerHeight)')
            await page.wait_for_timeout(500)
            # Action 4
            print(f"Scrolling down by one page height (Step 2, Action 3)")
            await page.evaluate('window.scrollBy(0, window.innerHeight)')
            await page.wait_for_timeout(500)

            # --- Step 3 ---
            # Action 5
            await _try_locate_and_act(page, "xpath=//html/body/main/div/div[4]/form/div[1]/div[1]/div/div/input", "fill", text=replace_sensitive_data("Spencer", SENSITIVE_DATA), step_info="Step 3, Action 1")
            # Action 6
            await _try_locate_and_act(page, "xpath=//html/body/main/div/div[4]/form/div[1]/div[2]/div/div/input", "fill", text=replace_sensitive_data("Willett", SENSITIVE_DATA), step_info="Step 3, Action 2")
            # Action 7
            await _try_locate_and_act(page, "xpath=//html/body/main/div/div[4]/form/div[1]/div[3]/div/div/input", "fill", text=replace_sensitive_data("spencer.willett15@gmail.com", SENSITIVE_DATA), step_info="Step 3, Action 3")
            # Action 8
            await _try_locate_and_act(page, "xpath=//html/body/main/div/div[4]/form/div[1]/div[4]/div/div/input", "fill", text=replace_sensitive_data("858-319-9931", SENSITIVE_DATA), step_info="Step 3, Action 4")

            # --- Step 4 ---
            # Action 9
            print("\n--- Task marked as Done by agent (Step 4, Action 1) ---")
            print(f"Agent reported success: True")
            # Final Message from agent (may contain placeholders):
            final_message = replace_sensitive_data("Task completed successfully. I have navigated to the job application page, scrolled down three times, and filled in the personal information fields as requested (First Name: Spencer, Last Name: Willett, Email: spencer.willett15@gmail.com, Phone: 858-319-9931). I have not submitted the form as per the instructions.", SENSITIVE_DATA)
            print(final_message)
